...
import time
import requests
import logfire # Importando o Logfire
import logging # Importando o logging
from logging import basicConfig, getLogger # Configurando o Logfire
...
# Carregando as variáveis de ambiente primeiro
load_dotenv()

# Configurando o Logfire depois
logfire.configure() # O token deve estar no .env como LOGFIRE_TOKEN=seu_token_aqui
basicConfig(handlers=[logfire.LogfireLoggingHandler()])
logger = getLogger(__name__) # Pegando o logger
logger.setLevel(logging.INFO) # Configurando o nível de log
logfire.instrument_requests() # Log para as requisições
logfire.instrument_sqlalchemy() # Log para o SQLAlchemy
...
def save_postgres_data(dados):
    ...
    logger.info(f'[{dados["timestamp"]}] Dados salvos no banco de dados PostgreSQL com sucesso')

# print(transform_data_bitcoin(extract_data_bitcoin()))

if __name__ == '__main__':
    create_table()
    logger.info('Iniciando ETL com atualização de dados a cada 15 segundos (CTRL + C para interromper)')
    while True:
        try:
            dados_json = extract_data_bitcoin()
            if dados_json:
                dados_tratados = transform_data_bitcoin(dados_json) # Transformando os dados
                logger.info(f'Dados tratados com sucesso, {dados_tratados}')
